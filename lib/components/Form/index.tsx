...
import {FormProps, PathSchema, StrictSchema, Schema} from "../../utils/src/types.ts";
...
function buildFormFromPathSchema<T = any, S extends StrictSchema = Schema> (
    ...
    handleAddField: (event: React.MouseEvent<HTMLButtonElement>, key: string, schema: S) => void,
    level: number
) {
    return (
        <>
            {
                pathSchema[SCHEMA_KEY].type === NUMBER_TYPE || pathSchema[SCHEMA_KEY].type === STRING_TYPE ?(
                    <input
                    ...
                    <>
                        {
                            Object.keys(pathSchema).map((key) => {
                                ...
                                if (typeof get(pathSchema, [key]) === OBJECT_TYPE && formData && get(formData, [key]) !== undefined) {
                                    return (
                                        <div key={key} style={{marginLeft:`${level * 10}px`}}>
                                            <label>{key}: </label>
                                            {buildFormFromPathSchema(get(pathSchema, [key]), get(formData, [key]), handleInputChange, handleAddField, level + 1)}
                                        </div>
                                    );
                                }
                                return null;
                        ...
                        }
                        {
                            ...
                            <button
                                onClick={(event) =>
                                    handleAddField(event, pathSchema[NAME_KEY], pathSchema[SCHEMA_KEY] as S)}>
                                Add Field
                            </button>
                        }
                    </>
...
export const Form = (props:FormProps) => {
    ...
    const [pathSchema, setPathSchema] = useState(toPathSchema(props.schema, '', formData));
    ...
    const handleAddField = (event: React.MouseEvent<HTMLButtonElement>, key: string, schema: StrictSchema) => {
    ...
    return (
        <form>
            {buildFormFromPathSchema(pathSchema, formData, handleInputChange, handleAddField, 0)}
        </form>
    );
};
...
import {ITEMS_KEY, NAME_KEY, PROPERTIES_KEY, SCHEMA_KEY} from "../constants.ts";
    ...
    if(ITEMS_KEY in schema && Array.isArray(formData)) {
        ...
        if (Array.isArray(schemaItems)) {
            formData.forEach((element, i) => {
                if (schemaItems[i]) {
                    pathSchema[i] = toPathSchemaInternal<T, S>(schemaItems[i] as S, `${name}[${i}]`, element);
                }
            });
        } else {
            formData.forEach((element, i) => {
                pathSchema[i] = toPathSchemaInternal<T, S>(schemaItems as S, `${name}[${i}]`, element);
            });
        }
    } else if (PROPERTIES_KEY in schema) {
        for (const property in schema.properties) {
            const field = get(schema, [PROPERTIES_KEY, property])
            pathSchema[property] = toPathSchemaInternal<T, S>(field as S, `${name}.${property}`, get(formData, [property]));
        }
    }
